...
This table maps shorthand taxon codes used during underwater visual surveys (UVS) to authoritative taxonomic identifiers and names. It serves as a translation layer between diver-entered field codes and the canonical taxonomy in `taxa_info.fish`.
...
#| tbl-cap: "Schema for taxa_info.uvs_fish_codes — Mapping Field Codes to Accepted Taxonomy"
...
bq_table_create(bq_table(project_id, "taxa_info", "uvs_fish_codes"),
                fields = uvs_fish_codes_schema,
                description = "Field codes used to enter fish observations from BLT surveys. Includes matching scientific name and AphiaID")
```
...
all_taxa <- all_taxa |> 
  add_row(taxon = "Plectroglyphidodon lacrymatus") |> 
  add_row(taxon = "Plagiotremus")
```
...
fb_manual <- read_csv(file.path(prj_path, "data/raw/taxonomy/fish/fb_missing_taxa.csv")) |> 
...
final_taxa$fb_spec_code[final_taxa$taxon == "Pteragogus turdus"] <- 70807
```
...
dupes_resolved <- read_csv(file.path(prj_path, "data/raw/taxonomy/fish/taxon_code_dupes.csv"))
...
```{r}
write_csv(final_taxa, file.path(prj_path, "data/output/taxa_info.uvs_fish_codes.csv"))

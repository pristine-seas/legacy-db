...
The `taxa_info.fish` table provides a standardized reference for all fish taxa observed during Pristine Seas expeditions. It harmonizes scientific names, taxonomic hierarchy, and ecological traits to support robust analysis, reporting, and integration across survey methods.
...
- ##### Parravicini et al., 2020
    ...
    A global gut content data synthesis leveraging machine learning to delineate trophic guilds of tropical reef fish. 
...
These fields define the accepted scientific identity and taxonomic lineage of each record (@tbl-taxonomy-schema). 

```{r eval = T, include = T}
#| label: tbl-taxonomy-schema
#| tbl-cap: "Taxonomic lineage fields for fish taxa in the Pristine Seas Database."

core_taxonomy_schema <- tribble(~field,              ~type,     ~required, ~description,
...
```{r taxonomy}
fish_taxa <- read_csv(file.path(prj_path, "data/output/lookup.uvs_fish_codes.csv"))
...
write_csv(fish_taxa, 
          file.path(prj_path, "data/processed/taxonomy_fish_staging.csv"))
```
...
common_name_schema <- tribble(~field,           ~type,    ~required, ~description,
...
```{r }
fish_taxa <- read_csv(file.path(prj_path, "data/processed/taxonomy_fish_staging.csv"))
...
#| tbl-cap: "Schema for trophic traits in `taxa_info.fish`."

trophic_traits_schema <- tribble(~field,            ~type,    ~required, ~description,
                                 "trophic_group",   "STRING", FALSE,     "Broad ecological role (Akiona et al., 2025 + internal classifications)",
                                 "trophic_guild",   "STRING", FALSE,     "Finer scale trophic guild (Parravicini et al., )",
                                 ...
                                 "feeding_path",    "STRING", FALSE,     "Primary foraging environment, e.g., 'benthic', 'pelagic' (FishBase).",
...
```{r trophic}
# Trophic Guilds (Parravicini et al.,)

trophic_guilds <- read_csv(file.path(prj_path, "data/raw/taxonomy/fish/Parravicini_et_al_pbio.3000702.s004.csv")) |> 
  clean_names() |> 
  mutate(taxon_name = str_replace_all(species, "_", " ")) |> 
  select(family, taxon_name, trophic_guild = trophic_guild_predicted_text)

names_to_aphiaID <- fish_taxa |> 
...
trophic_guilds <- trophic_guilds |> 
  left_join(names_to_aphiaID, 
            by = c("taxon_name" = "lookup_name"))

fish_taxa <- fish_taxa |> 
  left_join(trophic_guilds |> 
              select(accepted_aphia_id, trophic_guild)) |> 
  mutate(trophic_guild = case_when(order == "Carcharhiniformes"                                                                         ~ "shark",
                                   genus %in% c("Sphyraena", "Thunnus", "Acanthocybium", "Caranx", "Gymnosarda", "Sarda", "Katsuwonus") ~ "piscivore",
                                   genus %in% c("Eviota", "Trimma", "Pleurosicya", "Bryaninops", "Priolepis", "Hippocampus")            ~ "microinvertivore",
                                   genus %in% c("Rhincodon", "Mobula")                                                                  ~ "planktivore",
                                   genus == "Kyphosus"                                                                                  ~ "Herbivores Microvores Detritivores",
                                   TRUE ~ trophic_guild) |> 
           str_to_lower())

# Trophic Group (Akiona et al, 2025 + expert review)

## Akiona et al

akiona_df <- readxl::read_excel(file.path(prj_path, "data/raw/taxonomy/fish/Akiona_et_al_bb29484643_1_1.xlsx"),
...
extra_troph_info <- bind_rows(read_csv(file.path(prj_path, "data/raw/taxonomy/fish/Cleaned_AllFish_20240130.csv")) |>
                              ...
                              read_csv(file.path(prj_path, "data/raw/taxonomy/fish/trophic_info_filled_missing_PNG_JEC.csv")) |> 
                                select(accepted_name, trophic_group)) |> 
  distinct() |> 
  ...
  distinct() |> 
  mutate(trophic_group_manual = str_to_lower(trophic_group_manual) |> 
           str_replace("-", "_") |> 
           str_replace(" ", "_") |> 
           str_replace("/", " | "))
...
fish_taxa <- fish_taxa |> 
  ...
  mutate(trophic_group = str_to_lower(coalesce(trophic_group, trophic_group_manual))) |> 
  select(-trophic_group_manual)
```

```{r fb}
# FishBase information (trophic level, feeding path, max length)
...
# FishBase ecology info (feeding strategy)
...
fish_taxa <- fish_taxa |> 
  mutate(trophic_group = case_when(is.na(trophic_group) & feeding_type == "grazing on aquatic plants"                             ~ "herbivore | detritivore",
                                   is.na(trophic_group) & feeding_type %in% c("selective plankton feeding", "filtering plankton") ~ "planktivore",
                                   family == "Carcharhinidae"                                                                     ~ "shark",
                                   family == "Muraenidae"                                                                         ~ "lower_carnivore",
                                   genus %in% c("Trimma", "Eviota", "Pleurosicya", "Parioglossus")                                ~ "planktivore",
                                   TRUE ~ trophic_group)) 

fish_taxa$trophic_guild[fish_taxa$trophic_guild == "herbivores microvores detritivores"] <- "herbivore | microvore | detritivore"
fish_taxa$trophic_guild[fish_taxa$trophic_guild == "sessile invertivores"] <- "sessile_invertivore"
```
...
#| tbl-cap: "Schema for morphometric traits in `taxa_info.fish`."

morphometrics_schema <- tribble(~field,            ~type,    ~required, ~description,
...
missin_ab <- tibble::tribble(~accepted_name,               ~a,       ~b,   
                             "Coris schroederii",          0.0175,   2.957, # using C. batuensis
...
fish_taxa$ltl_ratio[fish_taxa$accepted_name == "Coris schroederii"] <- 1
...
fish_taxa |> filter(is.na(lw_a), rank == "species")
```
...
```{r}
habitat_schema <- tribble(~field,          ~type,    ~required, ~description,
                          "habitat_zone",  "STRING", FALSE,      "Spatial habitat classification based on FishBase")
...
```{r}
fisheries_schema <- tribble(~field,              ~type,    ~required, ~description,
                            "fishery_importance","STRING", FALSE,     "FishBase classification of the species' importance to fisheries")
...
# library(fishstat)
# 
# fao_capture <- merge(merge(merge(merge(merge(merge(capture, area), country), measure), source), species), status)
# 
# fao_species <- fao_capture |> 
#   distinct(species_name, scientific, isscaap)
# 
# names_to_aphiaID <- fish_taxa |> 
#   select(accepted_aphia_id, taxon_name, accepted_name) |>
#   pivot_longer(cols = c("taxon_name", "accepted_name"), names_to = "orig_col", values_to = "lookup_name") |>
...
# 
# fao_species |> 
#   left_join(names_to_aphiaID, by = c("scientific" = "lookup_name")) |> filter(!is.na(accepted_aphia_id))
```

### Conservation Status (IUCN)

The `taxa_info.fish` table includes fields derived from the IUCN Red List to support biodiversity assessments and conservation planning. Each species is matched to its most recent listing (SIS ID) and assigned a standardized category (e.g., `LC`, `NT`, `VU`, `EN`, `CR`).

```{r}
iucn_schema <- tribble(~field,        ~type,    ~required, ~description,
...
```{r}
final_schema <-  tribble(~field,                ~type,   ~required,    ~description,                                                                 
...
fish_taxa_fields <- pmap(final_schema, 
...
# Create table
bq_table_create(bq_table("pristine-seas", "taxa_info", "fish"),
                fields = fish_taxa_fields,
                description = "Taxonomic and trait information for fish species in the Pristine Seas database.")
...
#| fig-height: 8
#| 
trophic_levels <- rev(c("herbivore | detritivore", "planktivore", "lower_carnivore", "top_predator", "shark"))

taxa_info.fish <- fish_taxa |>
...
taxa_info.fish |> 
  filter(rank == "species") |> 
  group_by(common_family, trophic_guild) |> 
  summarise(n = n()) |> 
  ungroup() |> 
  group_by(common_family) |> 
  mutate(total = sum(n)) |> 
  ...
  arrange(desc(n)) |> 
  ggplot()+
  geom_col(aes(x = fct_reorder(common_family, total), y = n, fill = trophic_guild)) +
  coord_flip()+
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)))+
  paletteer::scale_fill_paletteer_d("tidyquant::tq_dark", na.value = "grey")+
  labs(x = "", y = "", title = "Number of Species by Family and Trophic group", fill = "")+
  theme(plot.title.position = "plot")
```

...
inverts_stations_fields <- tribble(
  ...
  # Survey effort
  "n_transects",       "FLOAT",   TRUE,      "Number of transect done at the station",
  ...
  # Summary metrics
  "n_morphotaxa",      "INTEGER", FALSE,   "Total number of unique invertebrate morphotaxa observed at the station (on-transect only).",
  "count",             "INTEGER", FALSE,   "Mean number of invertebrates recorded across transects",
  "density_m2",        "FLOAT",  FALSE,    "Mean density of invertebrates recorded across transects (individuals/m²).",
...
inverts_obs_fields <- tribble(
  ...
  "transect_length_m", "FLOAT",   TRUE,      "Length of the transect - typically 50m for general surveys",
...
density_by_taxa_fields <- tribble(
  ~field,             ~type,     ~required, ~description,
  ...
  # Aggregated metrics
  "count",             "INTEGER", TRUE,   "Mean number of individuals observed across transects at the station.",
...
```{r}
inverts_density_schema <- density_by_taxa_fields |>
  mutate(mode = if_else(required, "REQUIRED", "NULLABLE")) |>
  transmute(name = field,
            type = type,
            mode = mode,

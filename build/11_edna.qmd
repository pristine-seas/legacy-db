...
edna_site_fields <- tribble(
  ...
  "target",         "STRING",  FALSE,     "Primary taxonomic target for eDNA analysis (e.g., `fish`, `sharks`, `all_taxa`)",
...
**`paired_site_id`** — Links eDNA sites to coordinated visual or acoustic surveys for comparative analysis and ground-truthing of detection methods.
:::
...
edna_sites_all_fields <- bind_rows(core_site_fields, edna_site_fields) |>
  mutate(group = case_when(
    ...
    field %in% c("habitat", "target", "paired_site_id", "site_photos") ~ "eDNA-Specific",
    field %in% c("n_stations", "n_samples")                     ~ "Summary",
...
::: {.callout-important title="Sample ID Convention"}
eDNA sample IDs follow the pattern: `ps_station_id_r###`

- `PNG_2024_edna_001_10m_r001` — First replicate at 10m station
- `PNG_2024_edna_001_10m_r002` — Second replicate at same station  
- `PNG_2024_edna_001_20m_r001` — First replicate at deeper station

This ensures traceability from site → station → individual sample for laboratory processing and analysis.
:::

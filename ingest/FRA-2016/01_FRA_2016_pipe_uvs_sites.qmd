...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2016-03-01"), as.Date("2016-03-30")))
```
...
```{r}
uvs_sites <- read_xls(file.path(legacy_path, "GIS", "Clipperton_sites.xls")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         site      = station,
...
uvs_sites <- uvs_sites |> 
  mutate(exp_id    = exp_id,
         ...
         habitat   = "fore_reef",
         exposure = if_else(site %in% c(4,6,7,8,10,11,14), "windward", "leeward"), # from Clipperton_coral_cover_2016_Nat_Geo.xlsx
  ...
  filter(date != "2016-03-18") |> 
  select(ps_site_id, exp_id, method, region, subregion, locality, 
         date, latitude, longitude, habitat, exposure, site_name, in_mpa, mpa_notes, team_lead)

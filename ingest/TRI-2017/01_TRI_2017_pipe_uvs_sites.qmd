...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2017-01-10"), as.Date("2017-02-12")))  
...
legacy_path <- file.path(ps_paths$science, "legacy data/Tristin/")
```
...
```{r}
uvs_sites <- read_xlsx(file.path(legacy_path, "Tristan 2017 PS Field metadata for Alan.xlsx"),
                      sheet = "Dive Surveys") |> 
  clean_names() |> 
  filter(!is.na(site_number)) |> 
  mutate(latitude = latitude*-1,
         longitude = longitude*-1) |>
  distinct(year, month, day, island, site_number, latitude, longitude)
...
uvs_sites <- uvs_sites |> 
  mutate(exp_id    = exp_id,
         ...
         subregion = str_to_sentence(island),
         in_mpa    = FALSE,

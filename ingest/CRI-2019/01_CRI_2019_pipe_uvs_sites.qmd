...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2019-03-01"), as.Date("2019-03-30")),
                   lat_min     = -12.3, 
                   lat_max     = -5,  
                   lon_min     = 155, 
                   lon_max     = 170)  
...
```{r uvs-import}
uvs_sites <- read_xls(file.path(legacy_path, "GIS", "Osa_metadata_quan.xls")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         subregion = island,
  ...
  mutate(exp_id    = exp_id,
         ...
         in_mpa    = if_else(mangt == "No-take", TRUE, FALSE),
         mpa_notes = if_else(in_mpa, "No-take", NA_character_),
...
uvs_sites$subregion[uvs_sites$subregion == "Cano"] <- "Isla del Caño"
uvs_sites$subregion[uvs_sites$subregion == "Punta Lloruna"] <- "Punta Llorona"

uvs_sites$locality[uvs_sites$ps_site_id %in% c("CRI_2019_uvs_007", "CRI_2019_uvs_019", "CRI_2019_uvs_025") ] <- "Bajo del Diablo"
uvs_sites$locality[uvs_sites$ps_site_id == "CRI_2019_uvs_018"] <- "Bajo Matador"
uvs_sites$locality[uvs_sites$ps_site_id == "CRI_2019_uvs_009"] <- "Paraiso"

uvs_sites <- uvs_sites |> 
  mutate(site_name = locality,
         locality  = coalesce(locality, subregion),
         habitat   = if_else(habitat == "offshore submerged reef", "bank", "rocky_reef"),
         exposure  = "unknown") 

uvs_sites$subregion[uvs_sites$subregion != "Isla del Caño"] <- "Corcovado coast"

...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2014-10-01"), as.Date("2014-10-30")))
```
...
```{r}
uvs_sites <- read_xls(file.path(legacy_path, "Rapa_gis", "Rapa_metadata.xls")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         site      = station,
...
uvs_sites <- uvs_sites |> 
  mutate(exp_id    = exp_id,
         ...
         in_mpa    = if_else(mangt == "Rahui", TRUE, FALSE),
         mpa_notes = if_else(in_mpa, "Rahui", NA_character_),
...
uvs_sites$subregion <- recode(uvs_sites$subregion,
                              "Rapa" = "Rapa Iti")

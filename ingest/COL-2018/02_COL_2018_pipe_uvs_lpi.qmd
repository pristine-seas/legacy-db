...
legacy_path <- file.path(ps_paths$science, "legacy data/Malpelo/")
```
...
```{r}
sections_raw <- read_xlsx(file.path(legacy_path, "Benthos/Benthos_Kike.xlsx"),
                          sheet         = "transsects",
                          range         = "C1:X2",
                          col_names     = FALSE,
                          .name_repair  = "minimal") |>
...
contacts_raw <- read_xlsx(file.path(legacy_path, "Benthos/Benthos_Kike.xlsx"),
                          sheet         = "transsects",
                          range         = "B6:X50",
                          col_names     = FALSE,
                          .name_repair  = "minimal")
...
lpi_taxa <- complete_lpi_obs |> 
  ...
  filter(!morphotaxon %in% c("Sand", "Sediment", "Turf", "Barren", "Coral rubble", "Hard coral - dead", "Hard coral - bleached", "EAM - Epilithic Algal Matrix", "Turf red", "Turf brown")) |> 
  arrange(desc(count))
...
lpi_taxa$taxon_clean[lpi_taxa$morphotaxon == "Pavona clavus + gigantea"] <- "Pavona"
...
erect_genera <- c("Caulerpa", "Stypopodium", "Hypnea", "Champia", "Plumaria", "Plocamium", "Botryocladia", "Chondrophycus", "Scinaia") 

new_taxa <- new_taxa |> 
  mutate(functional_group = case_when(genus %in% erect_genera |  family == "Scytosiphonaceae" ~ "algae_erect",
...
lpi_taxa$accepted_name[lpi_taxa$morphotaxon == "Ochrophyta sp. (encrusting)"] <-  "Ochrophyta"
lpi_taxa$rank[lpi_taxa$morphotaxon == "Ochrophyta sp. (encrusting)"] <- "phylum"
lpi_taxa$functional_group[lpi_taxa$morphotaxon == "Ochrophyta sp. (encrusting)"] <- "algae_encrusting"
...
transects <- lpi_observations |> 
  ...
  summarize(divers            = paste0(sort(unique(diver),decreasing = T), collapse = " | "),
            ...
            transect_length_m = n_sections*25,

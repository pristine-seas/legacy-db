...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2017-03-01"), as.Date("2017-03-30")))
```
...
```{r}
uvs_sites <- read_xls(file.path(legacy_path, "JF_GIS", "Metadata17.xls")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         site      = station,
...
uvs_exposures <- read_xlsx(file.path(legacy_path, "Fishes", "Selkirk_AMF.xlsx"),
                           sheet = "metadata") |> 
  clean_names() |> 
  mutate(exposure = str_to_lower(exposure),
         exposure = case_when(exposure == "protected" ~ "sheltered",
                              TRUE ~ exposure)) |>
  select(site = station, exposure)

uvs_sites <- uvs_sites |> 
  left_join(uvs_exposures, by = "site") 
...
uvs_sites <- uvs_sites |> 
  mutate(exp_id    = exp_id,
         ...
         subregion = case_when(subregion == "Selkirk" ~ "Alejandro Selkirk",
                               subregion == "Robinson" ~ "Robinson Crusoe",
                               TRUE ~ NA_character_),
...
uvs_sites$mpa_notes[uvs_sites$ps_site_id %in% c("CHL_2017b_uvs_017","CHL_2017b_uvs_018")] <- "El Palillo MPA"
uvs_sites$mpa_notes[uvs_sites$ps_site_id %in% c("CHL_2017b_uvs_023","CHL_2017b_uvs_024")] <- "Tierra Blanca MPA"

...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2014-03-30"), as.Date("2014-04-25")))
```
...
```{r}
uvs_sites <- read_xlsx(file.path(legacy_path, "GIS", "Moz_metadata.xlsx")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         site      = station,
...
uvs_sites_sf <- uvs_sites |>
  ...
  distinct(ps_site_id, region, subregion, locality, habitat, exposure, site_name, in_mpa, geometry)
...
mapview::mapview(uvs_sites_sf,
                 ...
                 popup = leafpop::popupTable(uvs_sites_sf, 
                                             zcol = c("ps_site_id", "region", "subregion", "locality", "habitat", "exposure", "site_name","in_mpa"))) |> 
...
uvs_sites |> 
  ...
  summarize(n_surveys = n(),
            .groups = "drop") |> 
  pivot_wider(names_from = subregion, values_from = n_surveys, values_fill = 0) |> 

...
legacy_path <- file.path(ps_paths$science, "legacy data/Galapagos/")

exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2015-12-01"), as.Date("2015-12-31")))
```
...
```{r}
uvs_sites <- read_xlsx(file.path(legacy_path, "GIS", "Metadata_Galapagos.xlsx"),
                      sheet = "Dives") |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
...
uvs_sites <- uvs_sites |> 
  mutate(exp_id    = exp_id,
         ...
         locality  = subregion,
         in_mpa    = TRUE,
         mpa_notes = "Galapagos National Park",
...
uvs_sites <- uvs_sites |> 
  mutate(subregion = case_when(str_detect(locality, "Wolf") ~ "Wolf",
                               str_detect(locality, "Darwin") ~ "Darwin",
                               str_detect(locality, "Isabela") ~ "Isabela",
                               locality == "Roca Rodonda" ~ "Isabela",
                               locality == "Seymour" ~ "Santa Cruz",
                               TRUE ~ subregion)) 

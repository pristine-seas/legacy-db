...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2014-09-01"), as.Date("2014-09-30")))
```
...
```{r}
uvs_sites <- read_xls(file.path(legacy_path, "GIS", "MPAs_new.xls")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         site      = station,
         locality  = reef,
         notes     = habitat_description) |> 
  distinct()
...
uvs_sites$habitat <- recode(uvs_sites$habitat,
                              "forereef" = "fore_reef", "backreef" = "back_reef", "fringing reef" = "fringing_reef", "patch reef" = "patch_reef")

uvs_sites <- uvs_sites |> 
  mutate(subregion = case_when(locality %in% c("Ngemelis", "Ngemelis_outside") ~ "Ngemelis",
                               locality == "Lighthouse"                        ~ "Ngederrak",
                               locality %in% c("Ulong", "Rebotel")             ~ "Ngerumekaol",
                               locality %in% c("Ngebard", "Ebill")             ~ "Ebill",
                               TRUE ~ locality)) 


uvs_sites$subregion[uvs_sites$subregion == "Ngermasch"] <- "Ngermesch"

site_names <- read_xlsx(file.path(legacy_path, "benthic", "Nat_Geo_palau_corals Yim.xlsx")) |> 
  clean_names() |> 
  mutate(site = parse_number(station),
         ps_site_id =  paste0(exp_id, "_uvs_", str_pad(site, 3, pad = "0"))) |>
  filter(!is.na(site_name), site_name != "outside", str_squish(site_name) != str_squish(area)) |> 
  filter(!(ps_site_id == "PLW_2014_uvs_025" &  site_name == "Wall South Shark City 2")) |> 
  distinct(ps_site_id, site_name) 

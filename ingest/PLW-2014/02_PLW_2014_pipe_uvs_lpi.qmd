...
legacy_path <- file.path(ps_paths$science, "legacy data/Palau/")
```
...
```{r}
sections_seagrass <- read_xlsx(file.path(legacy_path, "benthic/SEAGRASS TRANSECTS PALAU.xlsx"),
          range        = "B1:J1",
          col_names    = FALSE,
          .name_repair = "minimal") |> 
  t() |>
...
sections_raw <- read_xlsx(file.path(legacy_path, "benthic/BENTHIC TRANSECTS PALAU1.xlsx"),
                          range        = "B1:DZ1",
                          col_names    = FALSE,
                          .name_repair = "minimal") |>
  t() |>
...
sections_raw <- bind_rows(sections_raw, sections_seagrass)

sections <- sections_raw |> 
  mutate(site             = str_pad(parse_number(sample), 3, pad = "0"),
         transect         = str_remove(sample, "\\d+"),
         depth_m          = if_else(transect == "A", 20, 10),
         ps_site_id       = paste(exp_id, "uvs", site, sep = "_"),
         transect_section = "0-50") |> 
...
```{r}
seagrass_ids <- sections$section_id[sections$ps_site_id %in% c("PLW_2014_uvs_065", 
                                                              "PLW_2014_uvs_066", 
                                                              "PLW_2014_uvs_067",
                                                              "PLW_2014_uvs_068")] 


contacts_seagrass <- read_xlsx(file.path(legacy_path, "benthic/SEAGRASS TRANSECTS PALAU.xlsx"),
                               range        = "B2:J23",
                               col_names    = FALSE,
                               .name_repair = "minimal")

contacts_seagrass <- contacts_seagrass |>
  set_names(c("field_name", seagrass_ids)) |>
  mutate(morphotaxon = field_name) |> 
  select(morphotaxon, field_name, all_of(seagrass_ids)) |>
  mutate(across(all_of(seagrass_ids), as.character)) |>
...
contacts_raw <- read_xlsx(file.path(legacy_path, "benthic/BENTHIC TRANSECTS PALAU1.xlsx"),
                          range        = "B2:DZ98",
                          col_names    = FALSE,
                          .name_repair = "minimal") 


valid_ids <- sections$section_id[!sections$ps_site_id %in% c("PLW_2014_uvs_065", 
                                                              "PLW_2014_uvs_066", 
                                                              "PLW_2014_uvs_067",
                                                              "PLW_2014_uvs_068")] 
...
contacts <- bind_rows(contacts, contacts_seagrass)
...
```{r}
coral_contacts <- read_xlsx(file.path(legacy_path, "benthic/Nat_Geo_palau_corals_PICRC.xlsx"),
                        sheet       = "corals",
                        col_names   = TRUE) |> 
  janitor::clean_names() |> 
  rename(site     = st, 
         ...
         taxon    = genus_species,
         diver    = observer) |> 
  mutate(site             = str_pad(site, 3, pad = "0"),
         depth_m          = if_else(transect == "A", 20, 10),
         diver            = str_to_sentence(diver),
         diver            = case_when(diver == "Yim"     ~ "Yimnang Golbuu",
                                      diver == "Marine"  ~ "Marine Gouezo",
                                      diver == "Uly"     ~ "Dawnette Olsudong",
                                      TRUE               ~ NA_character_),
         ps_site_id       = paste(exp_id, "uvs", site, sep = "_"),
         depth_strata     = stratify(depth_m),
         ps_station_id    = paste(ps_site_id, station_suffix(depth_strata), sep = "_"),
         transect_section = "0-50",
         morphotaxon = str_to_sentence(taxon),
         section_id       = paste(ps_station_id, transect, transect_section, sep = "_")) |> 
  mutate(morphotaxon = recode(morphotaxon, 
                              "Unknowncoral"         = "Scleractinia spp.",
                              "??"                   = "Scleractinia spp.",
                              "Seriatopora hystix"   = "Seriatopora hystrix",
                              "Psammacora"           = "Psammocora",
                              "Podobaccia"           = "Podabacia",
                              "Fungiids?"            =  "Fungiidae",
                              "Cyloseris"            = "Cycloseris",
                              "Echynophyllia"        = "Echinophyllia",
                              "Echinopyllia"         = "Echinophyllia",
                              "Pocillopora heydouxi" = "Pocillopora eydouxi",
                              "Favio"                = "Favia",
                              "Physiogyra"           = "Physogyra",
                              "Plereogyra"           = "Plerogyra",
                              "Palaustrea"           = "Palauastrea",
                              "Coscinera"            = "Coscinaraea")) |> 
  group_by(ps_site_id, ps_station_id, diver, depth_strata, transect, depth_m, section_id, morphotaxon) |>
  summarise(count = sum(count, na.rm= T), .groups = "drop") |>
  filter(!morphotaxon %in% c("Soft coral", "0")) 

coral_sections <- coral_contacts |> 
  rename(coral_diver = diver) |>
  group_by(ps_station_id, section_id, coral_diver) |>
  summarize(coral_pts = sum(count, na.rm = TRUE),
            .groups  = "drop")
```
...
section_alignment <- lpi_sections |> 
  full_join(coral_sections |> 
              select(section_id, coral_pts, coral_diver),
            by = c("section_id")) |> 
...
corrections <- c(# Genus and species spelling corrections
                 ...
                 "verispora"                   = "versipora",
                 "Gonipora"                    = "Goniopora",
                 "Ircinia groga"               = "Ircinia sp. groga",
                 "Didemnum encrusting bluish"  = "Didemnum sp. encrusting bluish",
                 "Didemnidae gruixut cautxu"   = "Didemnum sp. gruixut cautxu",
                 "Didemnum verdós llavis"      = "Didemnum sp. verdos llavis",
                 "Didemnidae blanc"            = "Didemnum sp. blanc",
                 "Foraminifer"                 = "Foraminifera",
                 "Mega hidrari"                = "Hydrozoa",
                 "Sea fan shallow"             = "Gorgoniidae sp. shallow",
                 "Sea fans deep"               = "Gorgoniidae sp. deep",
                 "Soft coral"                  = "Octocorallia",
                 "Dictyosphaeria versluyssii"  = "Dictyosphaeria versluysii",
                 "Gorgònia fuet"               = "Gorgonia sp. fuet",
                 "Black hydrarian"             = "Hydrozoa sp. black",
                 "Taca marró fosc"             = "Phaeophyceae sp.",
                 "Xetospongia"                 = "Xestospongia",
                 "Zoantaria"                   = "Zoantharia",
                 "Actinia encrusting dark"     = "Actiniaria sp. dark",
                 "Ecteinascidia lila"          = "Ecteinascidia",
                 "Pseudochondrosia"            = "Porifera sp.")
...
lpi_taxa$taxon_clean[lpi_taxa$morphotaxon == "Halimeda cuneata f. digitata"] <- "Halimeda cuneata f. digitata"
lpi_taxa$taxon_clean[lpi_taxa$morphotaxon == "Halimeda cuneata f. undulata"] <- "Halimeda cuneata f. undulata"
lpi_taxa$taxon_clean[lpi_taxa$morphotaxon %in% c("Cca unidentified", "Other cca")] <- "Corallinales"
...
new_taxonomy_ranks <- furrr::future_map_dfr(new_worms_records$accepted_aphia_id,
  ...
  clean_names() |>
  select(accepted_aphia_id, kingdom, class, order, family, genus)
...
new_taxa <- new_taxa |> 
  mutate(functional_group = case_when(genus == "Cymodocea"                          ~ "seagrass",
                                      genus == "Sphacelaria"                        ~ "turf",
                                      genus == "Sargassum"                          ~ "algae_canopy",
                                      genus %in% seagrass_genera                    ~ "seagrass",
                                      ...
                                      family %in% c("Valoniaceae")                  ~ "algae_erect",
...
transects <- lpi_observations |> 
  ...
  summarize(divers            = paste0(sort(unique(diver),decreasing = T), collapse = " | "),
            ...
            transect_length_m = n_sections*50,
...
cover_by_station_group |> 
  ...
  scale_fill_manual(values = c(ps_colors("functional_groups"), "seagrass" = "cyan"))+
  labs(x = NULL, 

...
legacy_path <- file.path(ps_paths$science, "legacy data/Selvagen/")
```
...
```{r}
sections_raw <- read_xlsx(file.path(legacy_path, "Benthic/Benthic_Transects_Kike.xlsx"),
                          sheet         = "rough data",
                          range         = "B1:KF2",
                          col_names     = FALSE,
                          .name_repair  = "minimal") |>
...
contacts_raw <- read_xlsx(file.path(legacy_path, "Benthic/Benthic_Transects_Kike.xlsx"),
                          sheet         = "rough data",
                          range         = "B6:KF77",
                          col_names     = FALSE,
                          .name_repair  = "minimal")
...
ps_taxon_cleaner <- function(x) {
  ...
  x <- step("Handle 'cf.' vs 'aff.' uncertainty rules", function(v) {
    ...
    if (any(aff_idx)) {
      # Extract genus (first Latin token)
      out[aff_idx] <- sub("^([A-Za-z]+).*", "\\1", out[aff_idx])
    }

    # --- cf. → remove token cleanly, KEEP epithet -----------------
    # Removes: cf, cf., cf , cf. , cf.something, cf. something
    out <- gsub("\\bcf\\.?\\s*", "", out, ignore.case = TRUE)

    # Remove stray dots left behind from bad formatting
    out <- gsub("\\s*\\.\\s*", " ", out)
  ...
  x <- step("Remove trailing 'like' qualifiers", function(v) {
    trimws(gsub("\\blike\\b.*$", "", v, ignore.case = TRUE))
  })
...
lpi_taxa <- complete_lpi_obs |> 
  ...
  filter(!morphotaxon %in% c("Turf (unidentified)","Sand", "Sediment", "Turf", "Barren", "Coral rubble", "Hard coral - dead", "Hard coral - bleached", "EAM - Epilithic Algal Matrix")) |> 
  arrange(desc(count))

lpi_taxa <- lpi_taxa |> 
  mutate(taxon_clean = ps_taxon_cleaner(morphotaxon)) 

# 1. CCA and Maërl  → Corallinales
lpi_taxa$taxon_clean[grepl("\\bCCA\\b", lpi_taxa$morphotaxon, ignore.case = TRUE)] <- "Corallinales"
lpi_taxa$taxon_clean[grepl("\\bMaërl\\b", lpi_taxa$morphotaxon, ignore.case = TRUE)] <- "Corallinales"

# 2. Cyanobacteria → Cyanophyceae
lpi_taxa$taxon_clean[grepl("\\bCyanobacteria\\b", lpi_taxa$morphotaxon, ignore.case = TRUE)] <- "Cyanophyceae"

# 3. Sponge → Porifera  (matches “sponge” or “sponges”, but not inside words)
lpi_taxa$taxon_clean[grepl("\\bsponge(s)?\\b", lpi_taxa$morphotaxon, ignore.case = TRUE)] <- "Porifera"

# 4. Bryozoans → Bryozoa
lpi_taxa$taxon_clean[grepl("\\bbryozoan(s)?\\b", lpi_taxa$morphotaxon, ignore.case = TRUE)] <- "Bryozoa"

## 5. Special case: Cystoseira abies marina → Cystoseira abies-marina
lpi_taxa$taxon_clean[trimws(tolower(lpi_taxa$taxon_clean)) == "cystoseira abies marina"] <- "Cystoseira abies-marina"
```
...
new_worms_records$accepted_aphia_id[new_worms_records$taxon_clean == "Sargassum vulgare"] <- 494801
new_worms_records$accepted_name[new_worms_records$taxon_clean == "Sargassum vulgare"] <- "Sargassum vulgare"
...
new_taxa$phylum[new_taxa$class == "Chlorophyceae"   & is.na(new_taxa$phylum)] <- "Chlorophyta"
new_taxa$phylum[new_taxa$class == "Magnoliopsida"   & is.na(new_taxa$phylum)] <- "Tracheophyta"

# Functional groups

new_taxa <- new_taxa |> 
  mutate(functional_group = case_when(genus %in% c("Dictyota", "Padina", "Dasya", "Taonia", "Hypnea")   ~ "algae_erect",
                                      genus %in% c("Cladophora", "Sphacelaria", "Pseudotetraspora", "Pseudochlorodesmis", "Dasycladus", "Parvocaulis")           ~ "turf",
                                      family  == "Sargassaceae"            ~ "algae_canopy",
                                      phylum == "Porifera"                 ~ "sponges",
                                      genus %in% c("Distaplia", "Botrylloides", "Clavelina", "Cystodytes", "Symplegma") ~ "ascidians",
                                      # Corals and corallimorphs
                                      genus %in% c("Caryophyllia") ~ "hard_coral",        # solitary scleractinian
                                      genus %in% c("Corynactis")   ~ "other_cnidarians",  # corallimorpharian
                                      # Hydrozoans
                                      class %in% c("Hydrozoa")   ~ "hydrozoans",
                                      # Bryozoans
                                      phylum %in% c("Bryozoa") ~ "bryozoans",
                                      # Molluscs (bivalves)
                                      genus %in% c("Pinna")         ~ "molluscs",
                                      # Polychaetes
                                      genus %in% c("Sabella") | family ==  "Serpulidae"     ~ "worms",
                                      # Barnacles (sessile crustaceans)
                                      genus %in% c("Megabalanus", "Balanus", "Chthamalus", "Tetraclita") ~ "barnacles")) 
...
```{r}
fg_df <- read_xlsx(file.path(legacy_path, "Benthic/Benthic_Transects_Kike.xlsx"),
                          sheet         = "FUNCTIONAL 2",
                          range         = "A2:B72",
                          col_names     = FALSE,
                          .name_repair  = "minimal") |> 
  set_names(c("fg", "morphotaxon")) |> 
  distinct(morphotaxon, fg) |> 
  mutate(fg = case_when(fg == "CCA" ~ "cca",
                                      fg == "TURF"         ~ "turf",
                                      fg == "ERECT"        ~ "algae_erect",
                                      fg == "ENC"          ~ "algae_encrusting",
                                      fg == "CANOPY"       ~ "algae_canopy",
                        TRUE ~ fg))

lpi_taxa |> 
  left_join(fg_df) |> 
  filter(fg != functional_group, fg != "INVERT") |> 
  filter(functional_group != "cyanobacteria") #these are good
```
...
lpi_observations <- complete_lpi_obs |> 
  ...
  mutate(functional_group = case_when(morphotaxon %in% c("Barren", "Sediment", "Sand") ~ "sediment | rubble | barren", 
                                      morphotaxon %in% c("Turf", "Turf (unidentified)") ~ "turf",
...
lpi_observations |> filter(is.na(functional_group))
```
...
```{r}
lpi_observations |> count(functional_group)
```
...
lpi_stations <- transects |> 
  ...
  left_join(lpi_observations |>
              filter(transect_section != "Off", ! functional_group %in% c("sediment | rubble | barren", "turf")) |> 
...
```{r taxa_summary}
#| label: tbl-taxa
#| tbl-cap: "Top 10 taxa by frequency"
...
lpi_stations |> 
  select(ps_station_id, depth_strata, pct_algae_canopy, pct_cca, pct_cyano, 
         pct_turf, pct_algae_erect, pct_rubble) |> 
  pivot_longer(cols = -c(ps_station_id, depth_strata),
               names_to = "functional_group",
               values_to = "pct_cover") |> 
  mutate(functional_group = case_when(functional_group == "pct_algae_canopy" ~ "Canopy Algae",
         ...
         functional_group = factor(functional_group, 
                                   levels = c("Canopy Algae", "Crustose Coralline Algae",
...
lpi_stations |> 
  ggplot(aes(x = subregion, y = pct_algae_canopy)) +
  geom_quasirandom(aes(color = pct_algae_canopy), 
                   alpha = 0.4, 
                   size = 1.5,
  ...
  labs(title = paste(exp_id, ":", "Canopy Algae Cover by Subregions"),
       caption = "Each dot represents a survey station • Black diamonds show subregion averages • Gray bars indicate median values",
       x = NULL,
       y = "Canopy Algae Cover (%)",
       color = "Canopy Algae Cover (%)")

ggsave(filename = file.path(figures_out, paste0(exp_id, "_lpi_canopy_cover_by_subregion.png")),
       width = 14, height = 7, dpi = 300, bg = "white")
```
...
stations_csv <- validate_and_export(
  df        = lpi_stations |> 
    mutate(pct_eam = NA_real_, pct_seagrass = NA_real_,pct_soft_coral = NA_real_, pct_coral = NA_real_,notes = NA_character_),

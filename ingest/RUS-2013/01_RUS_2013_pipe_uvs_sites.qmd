...
legacy_path <- file.path(ps_paths$science, "legacy data/Franz_joseph_land")

exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2013-07-30"), as.Date("2013-08-30")))
```
...
```{r}
uvs_sites <- read_xls(file.path(legacy_path, "GIS_maps", "FJL_fish_metadata.xls")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         subregion = island,
...
uvs_sites$subregion[uvs_sites$subregion == "Cape Flora"] <- "Northbrook"
...
uvs_sites <- uvs_sites |> 
  ...
  select(ps_site_id, exp_id, method, region, subregion, locality, 
         date, time, latitude, longitude, exposure, site_name, in_mpa, mpa_notes, team_lead)


site_descr <- read_csv(file.path(legacy_path, "benthic_data", "consolidated_site_descriptions.csv"))

uvs_sites <- uvs_sites |> 
  left_join(site_descr |> select(ps_site_id, habitat, notes = note), by = "ps_site_id") |> 
  relocate(habitat, .before = exposure) 

uvs_sites$habitat[uvs_sites$site_name == "Rubini Rock"] <- "rockwall"
uvs_sites$habitat[uvs_sites$site_name == "Brosh"] <- "rockwall"
uvs_sites$habitat[uvs_sites$site_name == "Cape Flora"] <- "kelp-forest"
uvs_sites$habitat[uvs_sites$locality == "Hayes"] <- "kelp-forest"
uvs_sites$habitat[uvs_sites$locality == "Nansen"] <- "kelp-forest"

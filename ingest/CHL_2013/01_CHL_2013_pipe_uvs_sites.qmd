...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2013-02-10"), as.Date("2013-02-22")))
```
...
```{r}
uvs_sites <- read_xlsx(file.path(legacy_path, "Desventuradas_gis", "Desventuradas_metadata.xlsx")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         region    = study,
         ...
         notes = comment) |> 
  distinct() |> 
  mutate(site_name = paste0("SA", site),
         site =  row_number()) |> 
  arrange(date, time) 

uvs_sites$subregion <- "San Ambrosio"

exposed_sites <- paste0("SA",c(1,2,3,4,6,7,8,9, 18))
...
uvs_sites <- uvs_sites |> 
  mutate(exp_id    = exp_id,
         ...
         time      = as_hms(time),
         exposure  = if_else(site_name %in% exposed_sites, "exposed", "sheltered"),
...
```{r}
all_sites <- tbl(bq_connection, "uvs.sites") |> 
  filter(!is.na(exp_id)) |> 
  collect()
```

```{r}
all_sites_sites_sf <- all_sites |>
  st_as_sf(coords = c("longitude", "latitude"), crs = 4326) |>
  mutate(exp_id = factor(exp_id)) |> 
  distinct(exp_id, geometry)
pal <- as.character(paletteer::paletteer_d("ggsci::category20_d3"))

mapview::mapview(all_sites_sites_sf,
                 zcol        = "exp_id",
                 legend      = TRUE,
                 map.types   = "Esri.WorldImagery",
                 layer.name  = "exp_id",
                 col.regions = pal,
                 at          = levels(all_sites_sites_sf$exp_id),
                 popup       = leafpop::popupTable(all_sites_sites_sf, 
                                                   zcol = "exp_id")) |>
  leafem::addMouseCoordinates() |>
  leaflet.extras::addFullscreenControl()
```

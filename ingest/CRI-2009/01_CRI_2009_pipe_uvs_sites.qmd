...
legacy_path <- file.path(ps_paths$science, "legacy data/Cocos Is")

exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2009-09-01"), as.Date("2009-09-30")))
```
...
```{r}
uvs_sites <- read_xlsx(file.path(legacy_path, "cocos_gis", "stations1.xlsx")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         site_name = station) |> 
  distinct() |> 
  mutate(site_name = str_replace_all(str_to_lower(site_name), "_", " "))

uvs_sites$region <- "Cocos Island"
uvs_sites$subregion <- "Cocos Island"

# Dates

fish_blt <- read_xlsx(file.path(legacy_path, "fish_data", "BLT.xlsx")) |> 
  clean_names() |> 
    mutate(site_name = str_to_lower(str_replace_all(station, "_", " "))) |> 
  distinct(date, site_name) 

fish_spc <- read_xls(file.path(legacy_path, "fish_data", "spc.xls")) |> 
  clean_names() |> 
  filter(!is.na(number)) |> 
  mutate(site_name = str_to_lower(str_replace_all(station, "_", " "))) |> 
  distinct(date, site_name) |> 
  anti_join(fish_blt |> select(site_name)) |> 
  group_by(site_name) |>
  summarise(date = first(date)) |>
  arrange(site_name)

fish_stations <- bind_rows(fish_blt, fish_spc) |> 
  distinct(date, site_name)

uvs_sites <- uvs_sites |> 
  left_join(fish_stations, by = "site_name")

# Times

uvs_times <- read_xls(file.path(legacy_path, "fish_data", "COCOS_FISH_DATA_09_bz.xls"),
          sheet = "METADATA") |> 
  clean_names() |> 
  filter(!is.na(time)) |> 
  mutate(site_name = str_to_lower(str_replace_all(station, "_", " ")),
         time = sprintf("%04d", as.integer(time)),
         time = hms::as_hms(paste0(substr(time, 1, 2), ":", substr(time, 3, 4), ":00"))) |> 
  distinct(site_name, date, time)

uvs_times$site_name[uvs_times$site_name == "submerged rock"] <- "subrock"
uvs_times$site_name[uvs_times$site_name == "1"] <- "silverado"
...
uvs_sites <- uvs_sites |> 
  ...
  select(ps_site_id, exp_id, method, region, subregion, locality, 
         date, latitude, longitude, habitat, exposure, site_name, in_mpa, mpa_notes, team_lead, notes)


uvs_sites$habitat[uvs_sites$site_name == "alcyone"] <- "bank"
uvs_sites$habitat[uvs_sites$site_name == "maria pinnacle"] <- "pinnacle_reef"
uvs_sites$habitat[uvs_sites$site_name == "silverado"] <- "pinnacle_reef"
...
uvs_sites_sf <- uvs_sites |>
  ...
  distinct(site_name, geometry)

mapview::mapview(uvs_sites_sf,
                 zcol = "site_name",
                 legend = TRUE,
                 map.types = "Esri.WorldImagery",
                 layer.name = "site_name",

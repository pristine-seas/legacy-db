...
```{r}
sections_raw <- read_xlsx(file.path(legacy_path, "Benthos/Transsects Kike-Azores_NG.xlsx"),
                          sheet         = "ROUGH DATA",
                          range         = "D1:OX2",
                          col_names     = FALSE,
                          .name_repair  = "minimal") |>
...
contacts_raw <- read_xlsx(file.path(legacy_path, "Benthos/Transsects Kike-Azores_NG.xlsx"),
                          sheet         = "ROUGH DATA",
                          range         = "C6:OX143",
                          col_names     = FALSE,
                          .name_repair  = "minimal")
...
lpi_taxa <- lpi_taxa |> 
  mutate(taxon_clean = bdc::bdc_clean_names(morphotaxon)$names_clean,
         taxon_clean = coalesce(taxon_clean,
                                str_remove_all(morphotaxon, " sp\\.| spp\\.")))
...
new_worms_records <- purrr::map_dfr(new_taxa$taxon_clean[-c(55)], 
...
deduped <- dupes |> 
  filter(name_status == "accepted" | accepted_name == "Dictyota implexa") |> 
  select(-dupe_count) |> 
  distinct()
...
manual_taxa <- new_taxa |> 
  filter(is.na(aphia_id)) |> 
  select(morphotaxon, taxon_name)

manual_taxa$aphia_id[manual_taxa$taxon_name == "Predaea feldmannii subsp. azorica"] <- 558712
manual_taxa$rank[manual_taxa$taxon_name == "Predaea feldmannii subsp. azorica"] <- "subspecies"
manual_taxa$accepted_aphia_id[manual_taxa$taxon_name == "Predaea feldmannii subsp. azorica"] <- 558712
manual_taxa$accepted_name[manual_taxa$taxon_name == "Predaea feldmannii subsp. azorica"] <- "Predaea feldmannii subsp. azorica"
manual_taxa$status[manual_taxa$taxon_name == "Predaea feldmannii subsp. azorica"] <- "accepted"

manual_add <- get_taxonomic_ranks(558712) |> 
  clean_names() |> 
  select(accepted_aphia_id, kingdom, class, order, family, genus) |> 
  mutate(phylum = "Rhodophyta")

manual_taxa <- manual_taxa |>
  left_join(manual_add)

new_taxa <- new_taxa |>
  filter(!is.na(aphia_id)) |>
  bind_rows(manual_taxa)

# Functional groups

fg_df <- read_xlsx(file.path(legacy_path, "Benthos/Transsects Kike-Azores_NG.xlsx"),
                          sheet         = "ROUGH DATA",
                          range         = "A6:C143",
                          col_names     = FALSE,
                          .name_repair  = "minimal") |> 
  set_names(c("fg", "type", "morphotaxon")) |> 
  distinct(morphotaxon, fg)

new_taxa <- new_taxa |> 
  left_join(fg_df) |> 
  mutate(functional_group = case_when(fg == "CCA"          ~ "cca",
                                      fg == "TURF"         ~ "turf",
                                      ...
                                      # Hydrozoans
                                      genus %in% c("Plumularia")   ~ "hydrozoans",
                                      # Bryozoans
                                      genus %in% c("Reptadeonella") ~ "bryozoans",
                                      # Molluscs (bivalves)
...
new_taxa <- new_taxa |> 
  select(-pct_count, -morphotaxon, - fg) |>
  distinct()
...
stations_csv <- validate_and_export(
  df        = lpi_stations |> 
    mutate(pct_eam = NA_real_, pct_seagrass = NA_real_,pct_soft_coral = NA_real_, notes = NA_character_),

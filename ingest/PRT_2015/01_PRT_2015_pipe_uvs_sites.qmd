...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2015-09-01"), as.Date("2015-09-30")))
```
...
```{r}
uvs_sites <- read_xls(file.path(legacy_path, "GIS", "INSITU_METADATA.xls")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         site      = station,
...
uvs_sites <- uvs_sites |> 
  filter(!str_detect(ps_site_id, "A")) |> 
  mutate(subregion = if_else(subregion == "Grande", "Selvagem Grande", "Selvagem Pequena"))

...
legacy_path <- file.path(ps_paths$science, "legacy data/sala_y_gomez/salas_y_gomez")
```
...
```{r}
sections_SyG <- read_xlsx(file.path(legacy_path, "carlos_kike_Expedition_results/Calculos_Quique.xlsx"),
                              sheet        = "DATOS",
                              range        = "B1:CW2",
                              col_names    = FALSE,
                              .name_repair = "minimal") |>
  ...
  clean_names() |> 
  set_names(c("site_name", "transect_section")) |> 
  mutate(tmp_id = paste(site_name, transect_section, sep = "_"))

contacts_SyG <- read_xlsx(file.path(legacy_path, "carlos_kike_Expedition_results/Calculos_Quique.xlsx"),
                              sheet        = "DATOS",
                              range        = "A3:CW16",
                              col_names    = FALSE,
                              .name_repair = "minimal") |> 
  set_names(c("field_name", sections_SyG$tmp_id)) |> 
  mutate(morphotaxon = field_name) |> 
  select(morphotaxon, field_name, all_of(sections_SyG$tmp_id)) |>
  mutate(across(all_of(sections_SyG$tmp_id), as.character)) |> 
  pivot_longer(cols      = -c(field_name, morphotaxon),  # Exclude BOTH taxonomy columns
               names_to  = "tmp_id",
               values_to = "contacts") |>
  mutate(contacts    = parse_number(contacts),
         field_name  = str_to_sentence(field_name),
         morphotaxon = str_to_sentence(morphotaxon)) |>
  filter(!is.na(contacts)) |>
  select(tmp_id, field_name, morphotaxon, contacts)
```

```{r}
sections_rapa <- read_xlsx(file.path(legacy_path, "carlos_kike_Expedition_results/Calculos_Quique.xlsx"),
                          sheet        = "DATOS",
                          range        = "B35:CW36",
                          col_names    = FALSE,
                          .name_repair = "minimal") |>
...
contacts_rapa <- read_xlsx(file.path(legacy_path, "carlos_kike_Expedition_results/Calculos_Quique.xlsx"),
                              sheet        = "DATOS",
                              range        = "A37:CW66",
                              col_names    = FALSE,
                              .name_repair = "minimal") |> 
  set_names(c("field_name", sections_rapa$tmp_id)) |> 
  mutate(morphotaxon = field_name) |> 
  select(morphotaxon, field_name, all_of(sections_rapa$tmp_id)) |>
  mutate(across(all_of(sections_rapa$tmp_id), as.character)) |> 
  pivot_longer(cols      = -c(field_name, morphotaxon),  # Exclude BOTH taxonomy columns
               names_to  = "tmp_id",
               values_to = "contacts") |>
  mutate(contacts    = parse_number(contacts),
...
```{r}
sections <- sections_SyG |> 
  bind_rows(sections_rapa) |> 
  mutate(depth_m = parse_number(transect_section),
         section = str_extract(transect_section, "[A-Za-z]$"),
         transect_section = case_when(section == "A" ~ "0-10",
                                      section == "B" ~ "10-20",
                                      section == "C" ~ "20-30",
                                      section == "D" ~ "30-40",
                                      section == "E" ~ "40-50")) |> 
  group_by(site_name) |>
  mutate(depth_run = data.table::rleid(depth_m),
         transect  = LETTERS[depth_run]) |> 
  ungroup() |> 
  left_join(uvs_sites |> 
              select(ps_site_id, site_name),
            by = "site_name") |> 
  select(ps_site_id, transect, depth_m, transect_section, site_name, tmp_id) |> 
  ungroup() |> 
  ...
  mutate(exp_id       = exp_id,
         ...
         depth_strata = if_else(station_depth_m >= 30,
                                "superdeep",
                                stratify(station_depth_m)),
...
contacts <- contacts_rapa |> 
  bind_rows(contacts_SyG) |> 
  left_join(sections |> 
              distinct(tmp_id, section_id)) |> 
  filter(!is.na(contacts)) |>
...
complete_lpi_obs <- lpi_contacts |> 
  ...
  ungroup() |> 
  select(ps_station_id, exp_id, diver, depth_m, transect, transect_length_m, transect_section, morphotaxon, count = contacts)
```

# 3. Taxonomy Resolution

```{r}
corrections <- c("P. Eydouxi" = "Pocillopora eydouxi",
                 "S. Furcatum" = "Sargassum furcatum",
                 "Hidraria" = "Hydrozoa sp.",
                 "Corallinaceae prima" = "Corallinaceae sp. prima",
                  "Cyanobacteria"= "Cyanophyceae spp.",
                 "Black sponge" = "Porifera sp. black")
...
lpi_taxa <- complete_lpi_obs |> 
  ...
  filter(!morphotaxon %in% c("turf","Bare rock", "Sand", "Sediment", "Turf", "Barren", "Coral rubble", "Rock", "Dead coral",
                             "Hard coral - dead", "Hard coral - bleached", "EAM - Epilithic Algal Matrix"),
         !str_detect(morphotaxon, "Turf|turf")) |> 
  arrange(desc(count))
...
new_worms_records <- purrr::map_dfr(new_taxa$taxon_clean, 
  ...
  filter(accepted_aphia_id != 137880) |> 
...
deduped <- dupes |> 
  filter(name_status == "accepted" | accepted_aphia_id == 377184) |> 
  select(-dupe_count) |> 
  distinct()
...
new_taxa$phylum[new_taxa$accepted_name == "Palmophyllum"   & is.na(new_taxa$phylum)] <- "Prasinodermatophyta"
...
erect_genera <- c("Dictyota", "Padina", "Dasya", "Taonia", "Hypnea", "Galaxaura", "Halimeda", "Caulerpa", "Dictyurus", "Rhipilia", "Acrosymphyton", "Predaea", "Titanophora", "Chondrophycus",
                  ...
                  "Rhipiliopsis", "Gigartina", "Pterocladiella", "Rosenvingea")
...
lpi_taxa |> 
  get_dupes(morphotaxon)

lpi_taxa |> filter(is.na(accepted_aphia_id))
```
...
lpi_observations <- complete_lpi_obs |> 
  left_join(lpi_taxa |> 
            ...
            by = "morphotaxon") |> 
  mutate(functional_group = case_when(morphotaxon %in% c("Barren", "Sediment", "Sand", "Bare rock", "Rock") ~ "sediment | rubble | barren", 
                                      str_detect(morphotaxon, regex("turf", ignore_case = TRUE)) ~ "turf",
                                      morphotaxon == "Dead coral" ~ "hard_coral_dead",
...
lpi_stations |> 
  ...
  group_by(subregion) |> 
  summarise(#n_stations         = n_distinc t(ps_station_id),
            #avg_n_taxa         = mean(n_taxa),
            across(contains("pct"), mean), 
            .groups = "drop") |> 
  mutate_if(is.numeric, round, 1) |> 
  pivot_longer(cols = -subregion,
               names_to = "functional_group",
               values_to = "mean_pct_cover") |> 
  pivot_wider(names_from = subregion,
              values_from = mean_pct_cover,
              values_fill = 0) 
```
...
cover_by_station_group |> 
  ...
  scale_fill_manual(values = c(ps_colors("functional_groups")))+
  labs(x = NULL, 
...
cover_by_station_group |> 
  ...
  facet_grid(~depth_strata, scales = "free")+
...
lpi_stations |> 
  select(ps_station_id, exposure, pct_cca, pct_coral,
         pct_turf, pct_algae_erect, pct_rubble) |> 
  pivot_longer(cols = -c(ps_station_id, exposure),
               names_to = "functional_group",
               values_to = "pct_cover") |> 
         ...
         functional_group = factor(functional_group, 
                                   levels = c( "Hard Coral","Crustose Coralline Algae",
                                              "Turf Algae", "Erect Macroalgae", 
                                               "Rubble"))) |> 
  ggplot(aes(x = exposure, y = pct_cover, fill = exposure)) +
  ...
  scale_fill_manual(values = ps_colors("exposure"))
...
lpi_stations |> 
  ...
  labs(title = paste(exp_id, ":", "Coral Cover by Subregions"),
       caption = "Each dot represents a survey station • Black diamonds show subregion averages • Gray bars indicate median values",
       x = NULL,
       y = "Coral Cover (%)",
       color = "Coral Cover (%)")

ggsave(filename = file.path(figures_out, 
                            paste0(exp_id, "_lpi_canopy_cover_by_subregion.png")),
       width = 14, height = 7, dpi = 300, bg = "white")
```
...
```{r}
lpi_stations$divers <- "Kike Ballesteros | Carlos Gaymer"
cover_by_station_taxa$divers <- "Kike Ballesteros | Carlos Gaymer"
```

...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2011-02-01"), as.Date("2011-03-30")))
```
...
```{r}
uvs_sites <- read_xls(file.path(legacy_path, "DATA", "METADATA_NEW1.xls")) |> 
  clean_names() |> 
  rename(latitude   = lat,
         longitude  = long,
         site_name  = station,
         ...
         subregion  = reef) |> 
  distinct() |> 
  filter(site_name != "ARCH") |> 
  mutate(region = if_else(subregion %in% c("RN"), "Rapa Nui", "Salas y Gomez"),
         subregion = if_else(subregion == "SR", "Bajo Scott", region),
         exp_id    = exp_id,
         method    = "uvs",
         ...
         # Exposure info from the inverts datasheet
         exposure = if_else(site_name %in% c("SG1", "SG4", "SG5", "RN3", "RN4", "RN5"), "leeward", "windward"),
         in_mpa = FALSE,
...
uvs_sites$habitat[uvs_sites$subregion == "Bajo Scott"] <- "bank"
uvs_sites$exposure[uvs_sites$subregion == "Bajo Scott"] <- "exposed"

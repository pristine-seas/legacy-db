...
legacy_path <- file.path(ps_paths$science, "legacy data/desventuradas/")
```
...
```{r}
sections_raw <- read_xlsx(file.path(legacy_path, "benthic_kike/Inventaris Desventuradas.xlsx"),
                          sheet        = "Rough data",
                          range        = "B1:GJ3",
                          col_names    = FALSE,
                          .name_repair = "minimal") |>
  ...
  row_to_names(1) |>
  clean_names() |> 
  set_names(c("site", "depth_m", "section")) 

sections_raw$site[sections_raw$site == "M01"] <- "M1"

sections <- sections_raw |> 
  mutate(depth_m = as.numeric(depth_m),
         site = str_remove(site, "M"),
         site_name = paste0("SA", site),
  ...
  left_join(uvs_sites |> 
            ...
            by = "site_name") |> 
  select(ps_site_id, transect, depth_m, transect_section, site_name) |> 
  ungroup() |> 
  ...
  select(exp_id, survey_type, ps_site_id, ps_station_id, method, diver,
         depth_m, station_depth_m, depth_strata,
         transect, transect_section, section_id)
```

```{r}
contacts_raw <- read_xlsx(file.path(legacy_path, "benthic_kike/Inventaris Desventuradas.xlsx"),
                          sheet        = "Rough data",
                          range        = "A4:GJ48",
                          col_names    = FALSE,
                          .name_repair = "minimal")
...
lpi_taxa <- complete_lpi_obs |> 
  ...
  filter(!morphotaxon %in% c("turf","Bare rock", "Sand", "Sediment", "Turf", "Barren", "Coral rubble", 
...
erect_genera <- c("Dictyota", "Padina", "Dasya", "Taonia", "Hypnea", "Galaxaura", "Halimeda", "Caulerpa", "Dictyurus", "Rhipilia", "Acrosymphyton", "Predaea", "Titanophora", "Chondrophycus",
                  ...
                  "Rhipiliopsis", "Gigartina", "Pterocladiella")
...
lpi_observations <- complete_lpi_obs |> 
  left_join(lpi_taxa |> 
            ...
            by = "morphotaxon") |> 
  mutate(functional_group = case_when(morphotaxon %in% c("Barren", "Sediment", "Sand", "Bare rock") ~ "sediment | rubble | barren", 
...
lpi_stations |> 
  group_by(exposure) |> 
  summarize(mean(pct_algae_canopy))
```
...
lpi_stations |> 
  select(ps_station_id, depth_strata, pct_cca, 
  ...
  mutate(functional_group = case_when(
         ...
         functional_group = factor(functional_group, 
                                   levels = c( "Crustose Coralline Algae",

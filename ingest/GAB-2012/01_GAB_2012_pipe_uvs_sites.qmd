...
legacy_path <- file.path(ps_paths$science, "legacy data/gabon")

exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2012-10-01"), as.Date("2012-10-31")))
```
...
```{r}
uvs_sites <- read_xlsx(file.path(legacy_path, "gabon_gis", "Metadata1.xlsx")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long) |> 
  distinct() |> 
  mutate(locality  = str_to_sentence(company),
         site_name = paste(str_to_sentence(name),locality, sep = "-")) 

uvs_sites$region <- "Gabon"
uvs_sites$subregion <- "Gabon"

# Times

uvs_times <- read_xlsx(file.path(legacy_path, "fish_data", "GABON_FISH_DATA.xlsx"),
          sheet = "META") |> 
  clean_names() |> 
  distinct(time, kike_codes) |> 
  filter(!is.na(kike_codes))

uvs_sites <- uvs_sites |> 
  left_join(uvs_times, by = c("kike_codes" = "kike_codes")) |> 
  arrange(date, time) |> 
...
uvs_sites |> 
  group_by(subregion, locality) |>
  summarize(n_surveys = n(),
            .groups = "drop") |> 
  pivot_wider(names_from = locality, values_from = n_surveys, values_fill = 0) |> 

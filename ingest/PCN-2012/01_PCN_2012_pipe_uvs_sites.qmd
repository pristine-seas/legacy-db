...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2012-03-01"), as.Date("2012-04-30")))
```
...
```{r}
uvs_sites <- read_xlsx(file.path(legacy_path, "Pitcairn GIS", "final_metadata.xlsx")) |> 
  clean_names() |> 
  rename(subregion = island,
         site_name = transect) |> 
  group_by(date, subregion, site_name, latitude, longitude, zone, wind) |> 
  summarize(time = min(time), .groups = "drop") |>
  arrange(date, time)

# lets excluse one netry
uvs_sites <- uvs_sites |> 
  filter(!(site_name == "DU10W" & latitude == -24.67600 & longitude == -124.7860))

uvs_sites$site_name[uvs_sites$site_name == "DU10W"] <- "DU10"
...
uvs_sites <- uvs_sites |> 
  mutate(exp_id    = exp_id,
         ...
         time      = as_hms(time),
         habitat   = if_else(zone == "LA", "patch_reef", "fore_reef"),
         exposure  = if_else(zone == "LA", "lagoon", str_to_lower(wind)),
         region    = "Pitcairn",
...
uvs_sites$exposure <- recode(uvs_sites$exposure, "protected"    = "sheltered")

uvs_sites$exposure[uvs_sites$site_name == "HE03"] <- "leeward"

uvs_sites |> count(exposure)
...
uvs_sites_sf <- uvs_sites |>
  ...
  distinct(ps_site_id, region, subregion, locality, habitat, exposure, in_mpa, site_name, geometry)
...
mapview::mapview(uvs_sites_sf,
                 ...
                 popup = leafpop::popupTable(uvs_sites_sf, 
                                             zcol = c("ps_site_id", "region", "subregion", "locality", "habitat", "exposure", "in_mpa", "site_name"))) |> 

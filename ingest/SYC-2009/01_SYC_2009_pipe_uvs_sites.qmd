...
legacy_path <- file.path(ps_paths$science, "legacy data/Seychelles/farquah")

exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2009-02-01"), as.Date("2009-02-28")))
```
...
```{r}
uvs_sites <- read_xlsx(file.path(legacy_path, "seychelles_gis", "farquhar_metadata_lat_long.xlsx")) |> 
  clean_names() |> 
  rename(latitude = lat,
         longitude = lon) |> 
  mutate(site = str_pad(station_num, 2, pad = "0"),
         site_name = paste0("F", site),
         date = lubridate::ymd(date),
         exposure  = str_to_lower(habitat) |> 
           str_replace_all(" ", "_")) |> 
  distinct(date, site_name, latitude, longitude, exposure) 

uvs_sites$exposure[uvs_sites$exposure == "patch_reefs"] <- "lagoon"

# Add time
uvs_times <- read_xls(file.path(legacy_path, "benthic_data", "Sites-Farq09.xls"),
         range = "B11:L44") |> 
  clean_names() |> 
  transmute(time = as_hms(time_1),
            site_name = name,
            notes = description)

uvs_sites <- uvs_sites |> 
  left_join(uvs_times, by = "site_name") |> 
  arrange(date, time) |> 
  mutate(exp_id    = exp_id,
         method    = "uvs",
         habitat   = if_else(exposure == "lagoon", "patch_reef", "fore_reef"),
         region    = "Seychelles",
         subregion = "Farquhar",

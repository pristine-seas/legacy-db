...
legacy_path <- file.path(ps_paths$science, "legacy data/Isla_de_los_estados/")

exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2018-02-12"), as.Date("2018-02-24")))
```
...
```{r uvs-import}
uvs_sites <- read_xls(file.path(legacy_path, "GIS", "METADATA_NEW.xls")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         site      = station,
  ...
  mutate(exp_id    = exp_id,
         ...
         subregion = case_when(subregion == "IdE" ~ "Isla de los Estados",
                               TRUE ~ "Peninsula Mitre"),
         in_mpa    = FALSE,
  ...
  select(ps_site_id, exp_id, method, region, subregion, locality, 
          latitude, longitude, habitat, exposure, site_name, in_mpa, mpa_notes, team_lead)

uvs_sites$exposure[uvs_sites$exposure == "protected"] <- "sheltered"

uvs_datetimes <- read_xlsx(file.path(legacy_path, "Benthos", "TdF_benthic_metadata.xlsx")) |> 
  clean_names() |> 
  distinct(station, time, date, comments) |> 
  transmute(ps_site_id =  paste0(exp_id, "_uvs_", str_pad(station,  3, pad = "0")),
            date = ymd(date),
            time = hms::as_hms(time),
            notes = str_to_sentence(comments))

uvs_sites <- uvs_sites |> 
  left_join(uvs_datetimes, 
            by = "ps_site_id") |> 
  relocate(date, .after = locality) |> 

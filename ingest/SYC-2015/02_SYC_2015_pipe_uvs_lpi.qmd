...
legacy_path <- file.path(ps_paths$science, "legacy data/Seychelles/Aldabra/")
```
...
```{r}
sections_raw <- read_xlsx(file.path(legacy_path, "Benthic/TRANSSECTS-càlculs.xlsx"),
                          sheet         = "ROUGH DATA",
                          range         = "A1:NQ2",
                          col_names     = FALSE,
                          .name_repair  = "minimal") |>
...
contacts_raw <- read_xlsx(file.path(legacy_path, "Benthic/TRANSSECTS-càlculs.xlsx"),
                          sheet         = "ROUGH DATA",
                          range         = "A4:NQ73",
                          col_names     = FALSE,
                          .name_repair  = "minimal")
...
read_coral_sections <- function(sheet, range) {
  readxl::read_xlsx(file.path(legacy_path, "Benthic/Benthos_Corals_Seychelles.xlsx"),
                    sheet = sheet,
                    range = range,
                    col_names = FALSE) |>
    janitor::clean_names() |>
    ...
    mutate(site       = stringr::str_pad(site, 3, pad = "0"),
           ...
           ps_site_id = paste(exp_id, "uvs", site, sep = "_"),
           transect   = if_else(depth_m == 20, "A", "B")) |>
    group_by(ps_site_id, transect) |>
...
coral_sections_aldabra <- read_coral_sections(sheet = "Aldabra_Abundance",
                                              range = "B2:EA3")

coral_sections_assumption <- read_coral_sections(sheet = "Assumption_Abundance",
                                                 range = "B2:BE3")

coral_sections_astove <- read_coral_sections(sheet = "Astove_Abundance",
                                             range = "B2:BS3")

coral_sections_cosmo <- read_coral_sections(sheet = "Cosmoledo_Abundance",
                                            range = "B2:DO3")

coral_sections <- bind_rows(coral_sections_aldabra, coral_sections_assumption, coral_sections_astove, coral_sections_cosmo)
```
...
coral_contacts_aldabra <- read_coral_contacts(sheet       = "Aldabra_Abundance",
                                              range       = "A4:EA310",
                                              sections_df = coral_sections_aldabra)

coral_contacts_assumption <- read_coral_contacts(sheet       = "Assumption_Abundance",
                                              range       = "A4:BE75",
                                              sections_df = coral_sections_assumption)

coral_contacts_astove <- read_coral_contacts(sheet       = "Astove_Abundance",
                                              range       = "A4:BS297",
                                              sections_df = coral_sections_astove)

coral_contacts_cosmo <- read_coral_contacts(sheet       = "Cosmoledo_Abundance",
                                              range       = "A4:DO294",
                                              sections_df = coral_sections_cosmo)

coral_contacts <- bind_rows(coral_contacts_aldabra, coral_contacts_assumption, coral_contacts_astove, coral_contacts_cosmo)
```
...
lpi_taxa <- complete_lpi_obs |> 
  ...
  filter(!morphotaxon %in% c("Turf (unidentified)","Sand", "Unidentified turf algae", "Filamentous algae",
...
# Functional groups
erect_genera <- c("Dictyota", "Padina", "Dasya", "Taonia", "Hypnea", "Galaxaura", "Halimeda", "Caulerpa", "Dictyurus", "Rhipilia")

new_taxa <- new_taxa |> 
  mutate(functional_group = case_when(accepted_name == "Melyvonnea erubescens"      ~ "cca",
                                      genus %in% erect_genera                       ~ "algae_erect",
                                      accepted_name == "Cladophoropsis fasciculata" ~ "algae_erect", 
...
lpi_observations <- complete_lpi_obs |> 
  ...
  mutate(functional_group = case_when(morphotaxon %in% c("Barren", "Sediment", "Sand") ~ "sediment | rubble | barren", 
                                      morphotaxon %in% c("Filamentous algae", "Unidentified turf algae", "Turf") ~ "turf",

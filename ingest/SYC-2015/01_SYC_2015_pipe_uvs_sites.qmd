...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2015-03-01"), as.Date("2015-03-30")))
```
...
```{r}
uvs_sites <- read_xlsx(file.path(legacy_path, "GIS", "NGS_seychelles_Metadata.xlsx")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         site      = station,
...
uvs_sites <- uvs_sites |> 
  mutate(exp_id    = exp_id,
         ...
         region    = "Aldabra Group",
         subregion = str_to_title(subregion),
         locality  = NA_character_,
         in_mpa    = if_else(mangt == "Protected", TRUE, FALSE),

...
legacy_path <- file.path(ps_paths$science, "legacy data/Revillagegido/")
```
...
```{r}
sections_raw <- read_xlsx(file.path(legacy_path, "Benthos/Transsects-Kike.xlsx"),
                          sheet         = "ROUGH DATA",
                          range         = "A1:HC2",
                          col_names     = FALSE,
                          .name_repair  = "minimal") |>
...
sections <- sections_raw |> 
  ...
  select(ps_site_id, depth_m, transect) |> 
...
contacts_raw <- read_xlsx(file.path(legacy_path, "Benthos/Transsects-Kike.xlsx"),
                          sheet         = "ROUGH DATA",
                          range         = "A6:HC61",
                          col_names     = FALSE,
                          .name_repair  = "minimal")
...
contacts <- contacts_raw |>
    ...
    select(field_name, all_of(valid_ids)) |>
    mutate(across(all_of(valid_ids), as.character)) |>
    pivot_longer(cols      = -c(field_name),  # Exclude BOTH taxonomy columns
                 names_to  = "section_id",
                 values_to = "contacts") |>
    mutate(contacts = parse_number(contacts)) |>
    filter(!is.na(contacts)) |>
    select(section_id, field_name, contacts)
```
...
corrections <- c(# Genus 
                 "durvillaei" = "durvillei",
                 "triquetrus" = "triqueter")
...
lpi_taxa <- complete_lpi_obs |> 
  ...
  filter(!morphotaxon %in% c("Mud","Sand", "Sediment", "Turf", "Barren", "Coral rubble", "Hard coral - dead",
                             "Hard coral - bleached", "EAM - Epilithic Algal Matrix", "Turf red", "Turf brown")) |> 
  arrange(desc(count))

clean_taxon <- function(x) {
  x <- gsub("\\?", "", x)                                             # remove question marks
  ...
  x <- gsub("\\s*\\([^)]*\\)", "", x)                                 # remove parentheses and contents
...
lpi_taxa$taxon_clean[lpi_taxa$morphotaxon == "Turf brown (Ectocarpaceae)"] <- "Ectocarpaceae"
lpi_taxa$taxon_clean[lpi_taxa$morphotaxon == "Turf red (Ceramiales)"] <- "Ceramiales"
lpi_taxa$taxon_clean[lpi_taxa$morphotaxon == "Cyanobacteria unidentified"] <- "Cyanophyceae"
lpi_taxa$taxon_clean[lpi_taxa$morphotaxon == "Encrusting bryozoan (white)"] <- "Bryozoa"
lpi_taxa$taxon_clean[lpi_taxa$morphotaxon == "Hydroids unidentified"] <- "Hydrozoa"

lpi_taxa$taxon_clean[lpi_taxa$morphotaxon == "Caulerpa racemosa v. occidentalis"] <- "Caulerpa racemosa var. occidentalis"
...
deduped <- dupes |> 
  filter(name_status == "superseded combination") |> 
  select(-dupe_count) |> 
  distinct()
...
new_taxa <- new_taxa |> 
  mutate(functional_group = case_when(genus %in% c("Caulerpa", "Siphonocladus") ~ "algae_erect",
                                      genus == "Chlorodesmis"                   ~ "turf",
                                      family == "Balanidae"                     ~ "barnacles",
                                      order == "Scleractinia"                   ~ "hard_coral",
                                      order == "Gelidiales"                     ~ "turf",
                                      order == "Peyssonneliales"                ~ "algae_encrusting",
                                      ...
                                      order == "Zoantharia"                     ~ "other_cnidarians",
                                      TRUE ~ NA_character_)) 
...
lpi_taxa <- lpi_taxa |> 
  filter(!(accepted_name == "Caulerpa chemnitzia" & rank == "variety"))
```
...
lpi_observations <- complete_lpi_obs |> 
  left_join(lpi_taxa |> 
            ...
            by = "morphotaxon") |> 
  mutate(functional_group = case_when(morphotaxon %in% c("Barren", "Sediment", "Sand", "Mud") ~ "sediment | rubble | barren", 
                                      morphotaxon %in% c("Turf", "Turf brown", "Turf red") ~ "turf",
...
lpi_stations <- transects |> 
  ...
  left_join(lpi_observations |>
              filter(transect_section != "Off", ! morphotaxon %in% c("Barren", "Sediment", "Turf", functional_group != "turf")) |> 
...
agent_transects <- transects |> 
  ...
  col_vals_equal(columns = vars(n_sections),
                   value = 5,
                   label = "One sections per transect",
  ...
  mutate(functional_group = case_when(functional_group == "pct_coral" ~ "Hard Coral",
         ...
         functional_group = factor(functional_group, 
                                   levels = c("Hard Coral", "Crustose Coralline Algae",
                                              "Turf Algae", "Erect Macroalgae", "Rubble"))) |> 
...
```{r}
stations_csv <- validate_and_export(df = lpi_stations |> 
...
observations_csv <- validate_and_export(df        = lpi_observations,
                                        table_ref = "uvs.lpi_observations",
                                        label     = "Observations",
                                        filename  = paste0(exp_id, "_uvs_lpi_observations.csv"))

cover_csv <- validate_and_export(df        = cover_by_station_taxa,

...
exp_config <- list(exp_id      = exp_id,
                   date_bounds = c(as.Date("2016-03-30"), as.Date("2016-04-30")))
```
...
```{r}
uvs_sites <- read_xls(file.path(legacy_path, "GIS", "REVILLA_NGS_2016.xls")) |> 
  clean_names() |> 
  rename(latitude  = lat,
         longitude = long,
         site      = station,
...
uvs_sites <- uvs_sites |> 
  mutate(exp_id    = exp_id,
         ...
         in_mpa    = TRUE,
         mpa_notes = "Revillagigedo - Zona Nucleo",
...
uvs_habitats <-  read_xlsx(file.path(legacy_path, "Benthos", "20160330 Revillagigedo Invertebrados Carlos.xlsx")) |> 
  clean_names() |>
  group_by(reef) |> 
  summarize(habitats = paste(unique(habitat), collapse = ", ")) |> 
  arrange(reef)

uvs_sites$subregion[uvs_sites$subregion == "San benidicto"] <- "San Benedicto"

uvs_sites$habitat[uvs_sites$ps_site_id %in% c("MEX_2016_uvs_001", "MEX_2016_uvs_003", "MEX_2016_uvs_008", "MEX_2016_uvs_009",
                                              "MEX_2016_uvs_011", "MEX_2016_uvs_012", "MEX_2016_uvs_013", "MEX_2016_uvs_017")] <- "wall"
